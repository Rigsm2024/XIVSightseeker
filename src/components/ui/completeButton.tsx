...
const CompleteButton = ({glog, filters, updateFilters}: completeProps) => {
    ...
    return (
        <div onClick={onClick} className='relative mr-2 w-6 h-6 cursor-pointer active:opacity-80'>
            ...
            <div className={`absolute right-0 z-10 w-5 h-5 top-0.5 left-0.5 rounded-full ${glog.IsCompleted ? 'bg-green-500' : 'bg-cardBase'}`}>
                {glog.IsCompleted && (
                    <div className='absolute top-0.5 left-1.5 border-r-2 border-b-2 border-white w-2 h-3 rotate-45'></div>
                )}
            </div>
